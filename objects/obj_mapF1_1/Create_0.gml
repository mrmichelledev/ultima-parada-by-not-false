	...
	for(var j = 0; j < tamanho_h; j++){
		...
		if(grade[# i,j] == 1){
			...
			if(inimigo1_max > 0){
				...
				if(irandom(troca_dir) == troca_dir and point_distance(x1, y1, obj_player.x, obj_player.y) > 700){
			...
			if(inimigo2_max > 0){
				...
				//deixa o spawn dos inimigos aleatorios e verifica se a distancia entre eles é superior a 1000
				if(irandom(troca_dir) == troca_dir and point_distance(x1, y1, obj_player.x, obj_player.y) > 500){
